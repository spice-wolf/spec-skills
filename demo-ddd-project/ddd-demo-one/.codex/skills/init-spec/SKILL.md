---
name: init-spec
description: Initialize markdown Specification files for Java projects, especially DDD-based systems, so they can be used later as context for Spec-Driven Development (SDD). Use when Codex needs to inspect an existing project, identify its architecture, domains, and reusable tools, then create the first batch of files under a specification root such as constitution.md, architecture.md, DDD.md, domain/*.md, and tool.md.
---
...
1. Ask for the Specification root directory first unless the user already provided it.
   Recommend `./specification`.
   If the user has no preference, use `./specification`.
...
7. Generate Specification files under the chosen root.
   Follow the file map and templates in [references/spec-templates.md](references/spec-templates.md).

8. If the user chooses step-by-step initialization, create and maintain `${specification-root}/TODO.md`.
...
Ask these questions in this order when the user did not already answer them:

1. `Specification 根目录放在哪里？建议使用 ./specification；如果你没有偏好，我将使用这个默认目录。`
2. After inspection: `当前识别到 <N> 个领域，判定为<大型/小型>项目。建议<分步骤/一次性>初始化。是否按这个方式继续？`

If the user already gave the root directory or mode, do not ask again. Respect the explicit choice.

## File Map

Generate files according to the following rules.

### `${specification-root}/constitution/constitution.md`
...
### `${specification-root}/architecture/architecture.md`
...
### `${specification-root}/architecture/DDD.md`
...
### `${specification-root}/domain/${specific-domain}.md`
...
### `${specification-root}/tool/tool.md`
...
1. Create `${specification-root}/TODO.md` before generating the first Specification file.
...
Finish only after:
- the root directory is confirmed or defaulted
